...
<table id="dtBasicExample" class="table table-striped table-bordered table-sm" >
  ...
  <c:forEach items="${ pageSimulationUnitaire }" var="elem">
    <tr>
      ...
      <td>   <a href="supprimerNdpDuListe?idSimulation=${SimulationModel.simulation.idSimulation }&ndp=${elem.id.ndp }"  class="btn btn-info" role="button"> Supprimer</a>   </td>
...
<li  > <a href="chercherNomenclature?simulation.idSimulation=${SimulationModel.simulation.idSimulation }&page=${page}&size=${size}&p=${item}&s=${s}"> ${item }  </a> </li>
 </c:forEach>
...
<table id="dtBasicExample" class="table table-striped table-bordered table-sm" >
  ...
  <c:forEach items="${pages}" var="item">
    <tr>            
      ...
      <td>  <a href="chercherNomenclature?simulation.idSimulation=${SimulationModel.simulation.idSimulation}&rechercheNDP=${item.ndp }&rechercheMotCle=${SimulationModel.rechercheMotCle }&page=${page}&size=${size}&p=${p}&s=${s}"  class="btn btn-info far fa-plus-square style='font-size:36px'" role="button"> </a>     
       </td>
...
<div class="panel panel-primary">
<div class="panel-heading"><h4>Compléter Les Informations</h4> </div>
<div  class="panel-body">
<label> Sauvegarder les choix </label>
 <a href="completerSimulationForm?idSimulation=${SimulationModel.simulation.idSimulation }"  class="btn btn-info" role="button">Sauvegarder, Compléter Les Informations </a>     
</div></div>
